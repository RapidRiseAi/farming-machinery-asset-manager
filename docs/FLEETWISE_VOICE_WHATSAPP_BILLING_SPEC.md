...
**Afrikaans quality levers (in order):** (1) canonical asset names + tenant aliases and application-side normalisation; (2) fuzzy entity resolution in our code; (3) consent-gated LLM interpretation only for unresolved commands; (4) plain-text Custom Speech evaluation later if real tests justify it. FleetWise always keeps `John Deere` in records/UI. Ollie Multilingual pronounces it correctly in the founder test; only the optional Willem fallback receives the speech-only `Djon Deer` rendering because Afrikaans neural voices do not support custom lexicons/phonemes. **Build a 200–500 utterance AF eval set before production shipping.**

**TTS:** Azure Ollie Multilingual for Afrikaans and English, selected after the founder voice test pronounced machinery names more naturally; Willem remains an available Afrikaans fallback. Afrikaans voices do not support a custom lexicon/phonemes, so pronunciation fixes are applied only to the spoken text in the app. **Prefer on-screen confirmation cards over speech** (visual beats audio next to a diesel engine).
