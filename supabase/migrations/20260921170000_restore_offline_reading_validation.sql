-- 20260921170000_restore_offline_reading_validation.sql
-- Put back four checks a rewrite dropped from the offline meter-reading path.
--
-- WHAT WAS LOST
-- =============================================================================
-- `apply_offline_capture` validated a replayed meter reading on four axes that
-- 20260920120000 no longer does. That migration rewrote the whole function to add the
-- `log_fuel` and `submit_checklist` branches, and in retyping it the `log_reading`
-- validation came back shorter:
--
--   not isfinite(v_date)                       a reading dated 'infinity'
--   v_date < date '1970-01-01'                   a reading dated 1969
--   v_machine.meter_type = 'none'               a reading against a machine with no meter
--   length(p_fields->>'name') > 200             an unbounded free-text name
--
-- Every other branch kept its length checks, which is what makes this a slip rather than a
-- decision: there is no comment anywhere saying these four were deliberately relaxed.
--
-- HOW IT GOT THROUGH
-- =============================================================================
-- `atomic_offline_capture.sql` has asserted all four since it was written. It fails on
-- PGlite well before reaching them, on the stubbed `digest()` that CLAUDE.md already
-- documents, so `pnpm db:check` reported a failure for an unrelated reason and the real
...
-- programmatically from that migration rather than retyped, because retyping a
-- three-hundred-line function is exactly what caused this.
...
begin
  ...
  if p_type = 'log_reading' then
    ...
    if v_reading is null or v_reading < 0 or v_reading > 99999999999.9
      or v_reading <> round(v_reading,1)
      -- Restored by 20260921170000. These four were on the 20260908 version and were lost
